...
rule all:
    input: 
        ...
        expand(os.path.join(out_dir, 'reports', f'{basename}.{search_type}.{alphabet}-k{{ksize}}.classification-report.csv'), ksize=ksizes),
        os.path.join(out_dir, 'reports', f'{basename}.{search_type}.classification-report.csv'),
        os.path.join(out_dir, 'reports', f'{basename}.{search_type}.classification-summaries.csv'),
...
rule assess_classification_each_ksize:
    ...
    output:
        classif_details = os.path.join(out_dir, 'reports', f'{basename}.{search_type}.{alphabet}-k{{ksize}}.classification-report.csv'),
        classify_fsummaries = os.path.join(out_dir, 'reports', f'{basename}.{search_type}.{alphabet}-k{{ksize}}.classification-summaries.csv')
    log: os.path.join(logs_dir, "assess-classification", f"{basename}.{search_type}.{alphabet}-k{{ksize}}.log")
    benchmark: os.path.join(logs_dir, "assess-classification", f"{basename}.{search_type}.{alphabet}-k{{ksize}}.benchmark")
    #conda: "conf/env/thumper.yml"
...
rule assess_classification_all:
    ...
    output:
        classif_details = os.path.join(out_dir, 'reports', f'{basename}.{search_type}.classification-report.csv'),
        classify_fsummaries = os.path.join(out_dir, 'reports', f'{basename}.{search_type}.classification-summaries.csv')
    log: os.path.join(logs_dir, "assess-classification", f"{basename}.{search_type}.{alphabet}.log")
    benchmark: os.path.join(logs_dir, "assess-classification", f"{basename}.{search_type}.{alphabet}.benchmark")
    #conda: "conf/env/thumper.yml"
    conda: "sourmash-dev.yml"
    shell:
        """
